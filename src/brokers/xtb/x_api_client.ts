...
const {on, EventEmitter, once} = require('events');
...
export class XApiClient {
  ...
  async login(): Promise<boolean> {
    ...
    const errorPromise = () =>
      once(this.#emitter, errorEventType) as Promise<Record<string, string>[]>;
    const successPromise = () =>
      once(this.#emitter, Constants.LOGIN) as Promise<Record<string, string>[]>;

    const [event] = await Promise.race([errorPromise(), successPromise()]);

    if (event.errorCode !== undefined) {
      throw new Error(`Could not log in: ${event.errorDescr} (${event.errorCode})`);
    }
  ...
  async *streamBalance(): AsyncGenerator<XApiClient.Balance> {
    yield* this.streamOperation('getBalance', Constants.STREAM_BALANCE) as AsyncGenerator<
      XApiClient.Balance
    >;
  }
  ...
  async *streamCandles(symbol: string): AsyncGenerator<XApiClient.Candle> {
    yield* this.streamOperation('getCandles', `${Constants.STREAM_CANDLES}_${symbol}`, {
      symbol: symbol,
    }) as AsyncGenerator<XApiClient.Candle>;
  }
  ...
  async *streamNews(): AsyncGenerator<XApiClient.News> {
    yield* this.streamOperation(Constants.STREAM_NEWS, 'getNews') as AsyncGenerator<
      XApiClient.News
    >;
  }
   ...
   */
  streamPing(): void {
    this.streamOperation(Constants.STREAM_PING, 'ping');
  }
  ...
  async *streamProfits(): AsyncGenerator<XApiClient.Profit> {
    yield* this.streamOperation(Constants.STREAM_PROFITS, 'getProfits') as AsyncGenerator<
      XApiClient.Profit
    >;
  }
  ...
  async *streamTickPrices(
    ...
    yield* this.streamOperation(
      ...
      'streamTickPrices',
      params
    ) as AsyncGenerator<XApiClient.TickPrice>;
  }
  ...
  async *streamTrades(): AsyncGenerator<XApiClient.Trade> {
    yield* this.streamOperation(Constants.STREAM_TRADES, 'getTrades') as AsyncGenerator<
      XApiClient.Trade
    >;
  }
   ...
   * waits for a reply
   */
  private async callOperation(customTag: string, operationName: string, args = {}): Promise<any> {
    this.checkLoggedIn();
    ...
    socket.send(JSON.stringify(payload));
    const [result] = await once(this.#emitter, operationName);
    return result;
  }
   ...
   * @param name the name of the socket, this will be used to route the async messages.
  ...
  private parseResponse(rcvd: string, socket: WebSocket): void {
    ...
    for (const response of responses) {
      ...
      if (responseObj.status) {
        const returnData =
          responseObj.customTag === Constants.LOGIN
            ? {streamSessionId: responseObj.streamSessionId}
      ...
      this.#emitter.emit(Constants.ERROR_PREFIX + responseObj.customTag, responseObj);
      console.warn(
  ...
  > {
    ...
    socket.send(JSON.stringify(payload));
    yield* on(this.#emitter, customTag);
  }
}
